            ...
            AfterSheet::class => function(AfterSheet $event) use(
                ...
                for ($i=0; $i < count($stamping_data_1); $i++) { 
                    if($stamping_data_1[$i]->stamping_cat == 1){
                        ...
                        $event->sheet->setCellValue('AN'.$start_col, $stamping_data_1[$i]->user->firstname);
                        $event->sheet->setCellValue('AO'.$start_col, $stamping_data_1[$i]->shift);
                        $event->sheet->setCellValue('AP'.$start_col, $stamping_data_1[$i]->prod_date);
                        $event->sheet->setCellValue('AQ'.$start_col, $stamping_data_1[$i]->prod_lot_no);
                        ...
                        $event->sheet->setCellValue('AU'.$start_col, $stamping_data_1[$i]->set_up_pins);
                        $event->sheet->setCellValue('AV'.$start_col, $stamping_data_1[$i]->adj_pins);
                        $event->sheet->setCellValue('AW'.$start_col, $stamping_data_1[$i]->qc_samp);
                        $event->sheet->setCellValue('AX'.$start_col, $stamping_data_1[$i]->prod_samp);
                        $event->sheet->setCellValue('AY'.$start_col, $stamping_data_1[$i]->total_mach_output);
                        ...
                        if (isset($stamping_data_1[$i]->stamping_ipqc)) {
                            $event->sheet->setCellValue('BB'.$start_col, $stamping_data_1[$i]->stamping_ipqc->ipqc_insp_name->firstname);
                        }
    
                        if (isset($stamping_data_1[$i]->oqc_details)) {
                            $event->sheet->setCellValue('BD'.$start_col, $stamping_data_1[$i]->oqc_details->inspector);
                            $event->sheet->setCellValue('BE'.$start_col, $stamping_data_1[$i]->oqc_details->date_inspected);
                            $event->sheet->setCellValue('BF'.$start_col, $stamping_data_1[$i]->oqc_details->num_of_defects);
                            $event->sheet->setCellValue('BG'.$start_col, ("=(AZ".$start_col."-BF".$start_col.")/AZ".$start_col.""));
    
                            if (isset($stamping_data_1[$i]->oqc_details->first_molding_info)){
                                $second_molding_packing_date = substr($stamping_data_1[$i]->oqc_details->first_molding_info->date_counted,0,10);
    
                                $event->sheet->setCellValue('BH'.$start_col, $stamping_data_1[$i]->oqc_details->first_molding_info->user_validated_by_info->firstname);
                                $event->sheet->setCellValue('BI'.$start_col, $second_molding_packing_date);
                                $event->sheet->setCellValue('BJ'.$start_col, $stamping_data_1[$i]->oqc_details->first_molding_info->shipment_output);
                            }
                        }
                    }   
                    // }
