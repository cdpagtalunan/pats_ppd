...
class CreateAssemblyRuncardStationsModsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('assembly_runcard_stations_mods', function (Blueprint $table) {
            ...
            $table->foreignId('assembly_runcards_id')->references('id')->on('assembly_runcards')->comment ='Id from assembly_runcards';
            $table->foreignId('assembly_runcard_stations_id')->references('id')->on('assembly_runcard_stations')->index('stations_foreign')->comment ='Id from assembly_runcard_stations';
